...
# Start health server as daemon thread IMMEDIATELY at import time
# so port 7860 is claimed before validator checks it
...
def run_task(task_name):
    ...
    try:
        while not done:
            ...
            action_str = f"rank({len(action.ranked_candidates)}_candidates)"
...
if __name__ == "__main__":
    # small pause to let health server start before inference begins
    time.sleep(2)
